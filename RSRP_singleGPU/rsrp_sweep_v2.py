...
def main(conf):
    ...
    conf = OmegaConf.merge({
        ...
        "run_name":     f"RSRP 2k{conf.seed} {conf.network_type} {time.strftime('%H:%M %m-%d')}"
    }, conf)
    # ES Config
...
def sweep(seed: int, conf_override: OmegaConf):
    def _objective(trial: optuna.Trial):
        conf = OmegaConf.merge(conf_override, {
            "seed": 0 + trial.number,
            # "seed": trial.suggest_categorical("seed", [1, 12, 15, 20])
    ...
    # # 定义网格搜索空间（仅 seed）
    # search_space = {
    #     "seed": [1, 12, 15, 20]  # 明确指定需要遍历的 seed 值
    # }
