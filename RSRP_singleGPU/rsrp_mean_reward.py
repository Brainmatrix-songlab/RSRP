...
def _runnner_run(runner: RunnerState, conf: ESConfig) -> Tuple[RunnerState, Dict]:
    ...
    weight = fitness - runner.previous_reward
    runner = runner.replace(previous_reward = jnp.mean(fitness))
...
def main(conf):
    conf = OmegaConf.merge({
        ...
        # Train & Checkpointing
        "total_generations": 2000,# for humanoid
        # "total_generations": 100,# for cartpole
    ...
    # Naming
    conf = OmegaConf.merge({
        "project_name": f"E-SNN-mean-reward-{conf.task}",
        "run_name":     f"RSRP {conf.seed} {conf.network_type} {time.strftime('%H:%M %m-%d')}"
    }, conf)
    # ES Config
    ...
    if "log_group" in conf:
        wandb.init(reinit=True, project=f"(G) E-SNN-mean-reward-{conf.task}", group=conf.log_group, name=str(conf.seed), config=OmegaConf.to_container(conf))
    else:
...
def sweep(seed: int, conf_override: OmegaConf):
    def _objective(trial: optuna.Trial):
        conf = OmegaConf.merge(conf_override, {
            ...
            "seed": 110 + trial.number,

            # "project_name": f"E-SNN-sweep",
            # "task": trial.suggest_categorical("task", ["humanoid", "inverted_pendulum"]),
            "es_conf": {
                # "lr":           trial.suggest_categorical("lr",  [0.3**i for i in range(5) ]),
