...
def main(conf):
    ...
    conf = OmegaConf.merge(conf_dict[task], conf)
    # Naming
    conf = OmegaConf.merge({
    ...
    if "log_group" in conf:
        wandb.init(reinit=True, project=f"(G) ESDense-{conf.task}-v2", group=conf.log_group, name=str(conf.seed), config=OmegaConf.to_container(conf))
    else:
...
def sweep(seed: int, conf_override: OmegaConf):
    def _objective(trial: optuna.Trial):
        conf = OmegaConf.merge(conf_override, {
            ...
            "es_conf": {
                "lr":           trial.suggest_categorical("lr",  [1e-2,3e-1,1e-1,3e-1,1e0,3e0,1e1]),
