...
def main(conf):
    ...
    # Naming
    conf = OmegaConf.merge({
        "project_name": f"E-SNN-no-crt-{conf.task}",
    ...
    if "log_group" in conf:
        wandb.init(reinit=True, project=f"(G) E-SNN-no-crt-{conf.task}", group=conf.log_group, name=str(conf.seed), config=OmegaConf.to_container(conf))
    else:
...
def sweep(seed: int, conf_override: OmegaConf):
    def _objective(trial: optuna.Trial):
        conf = OmegaConf.merge(conf_override, {
            ...
            "es_conf": {
                # "lr":           trial.suggest_categorical("lr",  [0.3**i for i in range(10) ]),
