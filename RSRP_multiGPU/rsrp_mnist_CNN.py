...
import rsrp
from rsrp import EvoState, EvoConfig, EvoStateInitConfig, NumpyDataLoader, PreAllocatedDataLoader, modules, optim, core, conf_trans, utils
...
conf = OmegaConf.merge(conf, {
    ...
    "pop_size": 8192,
    "subpop_num": 1,                    # serial computation when it is out of memory 
...
def main(conf):
    ...
    evo_conf = EvoConfig(
        network_cls=rsrp.modules.CNN(kernel_size=conf.kernel_size,cnn_channels=conf.channels,stride=conf.stride,mlp_features=conf.mlps),
    )
    ...
    evo_state = rsrp.state_init(key, evo_conf, init_conf)

    evo_conf = evo_conf.replace(
        optim_cls=rsrp.partition_optim_cls(evo_conf, evo_state.params),
    )
    if conf.wandb:
    ...
    for step in tqdm(range(1, conf.total_generations + 1)):       
        evo_state, fitness = rsrp.state_update(evo_state, evo_conf, train_data_loader)
        if (step-1)%20 == 0:
            eval = rsrp.evaluation(evo_state, evo_conf, test_data_loader.get_batch())
        metrics = {
            "fitness": np.sum(fitness),
            "Entrophy": rsrp.metrics.avg_entrophy(evo_state.params),
            "evaluation": eval,
        }
        if conf.wandb:
    ...
    for data_batch in test_data_loader:
        metrics["test_acc"] = rsrp.test(evo_state, evo_conf, data_batch)
