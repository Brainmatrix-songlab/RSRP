...
class LIF(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, V, V_thr=1.0, tau=1.5, V_reset=0.0):
        V_next = V + x - 1. / tau * (V - V_reset)
        x = jnp.where(V_next > V_thr, 1, 0)
...
class MlpBlock1(nn.Module):
    ...
    def __call__(self, x, V1, V2):
        ...
        x = 6 * self.Dense2(x)
        input_max = jnp.max(x[0])
        input_min = jnp.min(x[0])

        x, V2 = LIF()(x, V2)
        output_mean = jnp.mean(x[0])
        jax.debug.print('input_max:{input_max}', input_max=input_max)
        jax.debug.print('input_min:{input_min}', input_min=input_min)
        jax.debug.print('output_mean:{output_mean}', output_mean=output_mean)
        return x, V1, V2
...
class MixerBlock1(nn.Module):
    ...
    def setup(self):
        self.MlpBlock1 = MlpBlock(self.tokens_mlp_dim, name='token_mixing')
        self.MlpBlock2 = MlpBlock1(self.channels_mlp_dim, name='channel_mixing')
        self.layernorm1 = nn.LayerNorm()
        self.layernorm2 = nn.LayerNorm()
...
class MlpMixer_test(nn.Module):
    ...
    def __call__(self, inputs):
        ...
        for i in range(self.time_seq):
            ...
            x = self.layernorm(x)
            x = jnp.mean(x, axis=1)
            jax.debug.print('before:{before}', before=x)
            x = self.Dense(x)
            jax.debug.print('out:{out}', out=x)
            out += x
