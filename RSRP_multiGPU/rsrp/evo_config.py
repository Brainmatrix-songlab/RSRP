...
import rsrp
from rsrp import optim

def conf_trans(evo_conf,conf):

    evo_conf = evo_conf.replace(
        acc_cls=rsrp.metrics.accuracy,
        forward_cls=rsrp.batch_fwd,
    ...
    if conf.reward == "softrecall":
        evo_conf = evo_conf.replace(
            fitness_cls=rsrp.metrics.softrecall,
        )
    elif conf.reward == "softacc":
        evo_conf = evo_conf.replace(
            fitness_cls=rsrp.metrics.softacc,
        )
    elif conf.reward == "recall":
        evo_conf = evo_conf.replace(
            fitness_cls=rsrp.metrics.recall,
        )
    elif conf.reward == "accuracy":
        evo_conf = evo_conf.replace(
            fitness_cls=rsrp.metrics.accuracy,
        )
    elif conf.reward == "cross_entropy":
        evo_conf = evo_conf.replace(
            fitness_cls=rsrp.metrics.cross_entrophy,
        )
    # reward transform
    evo_conf = evo_conf.replace(
    ...
    if conf.EI_balance == "1:1":
        evo_conf = evo_conf.replace(
            network_cls=rsrp.modules.MLP_EI((conf.network_size, 10,),p=0.5),
        )
    elif conf.EI_balance == "1:1_input":
        evo_conf = evo_conf.replace(
            network_cls=rsrp.modules.MLP_EI_input((conf.network_size, 10,),p=0.5),
        )
    elif conf.EI_balance == "3:1":
        evo_conf = evo_conf.replace(
            network_cls=rsrp.modules.MLP_EI((conf.network_size, 10,),p=0.75),
        )
    elif conf.EI_balance == "2:1":
        evo_conf = evo_conf.replace(
            network_cls=rsrp.modules.MLP_EI((conf.network_size, 10,),p=2/3),
        )
    elif conf.EI_balance == "01":
        evo_conf = evo_conf.replace(
            network_cls=rsrp.modules.MLP_01((conf.network_size, 10,)),
        )
