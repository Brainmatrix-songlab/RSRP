...
wandb.init(project="stdp_mnist", name="train_seed_"+random_dir+str(random_num))
performance = None
...
if test_mode:
    ...
    weight_path = data_path + random_dir
    num_examples = 50000*4
...
num_evaluations = int(num_examples / update_interval)  # 18
validation_performance = np.zeros(num_evaluations)
...
for name in population_names:
    ...
    for conn_type in recurrent_conn_names:
        ...
        weightMatrix = get_matrix_from_file(weight_path + '../'+random_dir + connName + ending + '.npy')
...
#     b.savefig('raster_plot_%d.png' % fig_num)
...
while j < (int(num_examples)):
    ...
    progress = (j + 1) * 100.0 / num_examples
    if j % 100 == 0 or j == num_examples - 1:
        print "Progress: %.2f%% (%d/%d)" % (progress, j + 1, num_examples)
#     print 'run number:', j+1, 'of', int(num_examples)
    ...
    if np.sum(current_spike_count) < 5:
        ...
        if j % update_interval == 0 and j > 0:
            ...
            valid_spike_counts = np.zeros((validation_size, n_e))
            for k in xrange(validation_size):
                rates = testing['x'][k, :, :].reshape((n_input)) / 8. * input_intensity
                input_groups['Xe'].rate = rates
                b.run(single_example_time)
                valid_spike_counts[k, :] = np.asarray(spike_counters['Ae'].count[:]) - previous_spike_count
                previous_spike_count = np.copy(spike_counters['Ae'].count[:])
                input_groups['Xe'].rate = 0
                b.run(resting_time)

            valid_assignments = get_new_assignments(result_monitor[:], input_numbers[j - update_interval:j])
            valid_predictions = np.array(
                [get_recognized_number_ranking(valid_assignments, valid_spike_counts[k, :])[0] for k in
                 xrange(validation_size)])
            valid_correct = np.sum(valid_predictions == testing['y'][:validation_size].flatten())
            validation_performance[int(j / update_interval) - 1] = valid_correct / float(validation_size) * 100
            print 'Validation performance (Test as Val)', validation_performance[:(j / float(update_interval))]

            wandb.log({
                "train_acc": performance[int(j / update_interval) - 1],
                "val_acc": validation_performance[int(j / update_interval) - 1],
                "iteration": j,
...
if not test_mode:
    # save_theta()
    # save_connections()
    np.save(data_path + 'activity/performance.npy', performance)
    np.save(data_path + 'activity/validation_performance.npy', validation_performance)
else:
